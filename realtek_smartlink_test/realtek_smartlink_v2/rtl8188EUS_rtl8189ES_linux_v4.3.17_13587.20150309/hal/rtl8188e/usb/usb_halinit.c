	...
	)
{
	// RX Page Boundary	
	// 	
	u16 rxff_bndy = MAX_RX_DMA_BUFFER_SIZE_88E(Adapter)-1;
...
//		sMacAddr[5] = (u1Byte)GetRandomNumber(1, 254);
	...
	else
	{
		...
		_rtw_memcpy(pHalData->EEPROMMACAddr, &hwinfo[EEPROM_MAC_ADDR_88EU], ETH_ALEN);
	...
	)
{
	...
	Hal_EfuseParseMACAddr_8188EU(padapter, pHalData->efuse_eeprom_data, pHalData->bautoload_fail_flag);
#endif //CONFIG_EFUSE_CONFIG_FILE	
