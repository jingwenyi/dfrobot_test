...
{
    int32_t r, g1, g2, b, u, v, c, miny=CL_MIN_Y;//miny=38
...
void ColorLUT::iterate(IterPixel *ip, ColorSignature *sig)
{
    ...
    //这里能循环30次,这里是计算所有像素点落在一个区域的概率，
    //一步步把范围缩小
    for (scale=1<<30, sig->m_uMin=sig->m_uMax=sig->m_vMin=sig->m_vMax=0; scale!=0; scale>>=1)
    {
    	
        calcRatios(ip, sig, ratios);//计算比率
        if (ratios[0]>m_ratio) //m_ratio=0.8
        {
			sig->m_uMin += scale;
			//printf("jingwenyi sig->m_umin:%x,scale=%x\r\n",sig->m_uMin,scale);
		}
            
        else
        {
			sig->m_uMin -= scale;
			//printf("jingwenyi sig->m_umin:%x,scale=%x,log_i=%d\r\n",sig->m_uMin,scale,log_i);
		}
...
void ColorLUT::updateSignature(uint8_t signum)
{
    ...
    if (m_signatures[signum].m_type==CL_MODEL_TYPE_COLORCODE)
		...
		range = m_sigRanges[signum];//2.5 默认
	...
	//printf("signum=%d,m_uMin=%x,m_uMax=%x,m_vMin=%x,m_rgbSat=%x\r\n",signum,m_runtimeSigs[signum].m_uMin,
		//		m_runtimeSigs[signum].m_uMax,m_runtimeSigs[signum].m_vMin,m_runtimeSigs[signum].m_rgbSat);
}
...
int ColorLUT::generateLUT()
{
    ...
    for (r=0; r<CL_NUM_SIGNATURES; r++)
        updateSignature(r+1);//计算标志窗口的最大值和最小值
    //生成颜色通道，并标记识别区域
	//for(r=0;r<0xff;r+=4)
            ...
            for (b=0; b<1<<8; b+=1<<(8-CL_LUT_COMPONENT_SCALE))
            {
                ...
                for (sig=0; sig<CL_NUM_SIGNATURES; sig++)
                {
                    ...
                    if (m_signatures[sig].m_uMin==0 && m_signatures[sig].m_uMax==0)
                        continue;
					//printf("sig:%d,m_uMin:%x,m_uMin:%x;u=%x,v=%x\r\n",sig,m_signatures[sig].m_uMin,m_signatures[sig].m_uMax,u,v);
                    if ((m_runtimeSigs[sig].m_uMin<u) && (u<m_runtimeSigs[sig].m_uMax) &&
                            (m_runtimeSigs[sig].m_vMin<v) && (v<m_runtimeSigs[sig].m_vMax))
                    {
                 		//printf("jingwenyi--------generateLUT-------\r\n");
                 		//这里忽略了所有差值的低3 位，就是只要你的差值的高为相同，就算相同
                        u = r-g;
						...
						//printf("bin:%x,u=%x,v=%x\r\n",bin,u,v);

                        if (m_lut[bin]==0 || m_lut[bin]>sig+1)
                            m_lut[bin] = sig+1;//这里把0--7的图像识别编号到m_lut数组中
                    }
                }
            }
        }
