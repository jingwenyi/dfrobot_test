...
class CBlobAssembler {
    ...
    //当前准备加入的segment,它是一个activeblobs,
    //从左到右进行扫描中，寻找到的active blobs
    CBlob *currentBlob;

    // Pointer to pointer to current candidate, which is actually the pointer
    //当前blob的二级指针，其实际上是指向先前的next 变量
...
public:
    ...
    CBlob *finishedBlobs;//结束指针
    short maxRowDelta;  //最大的行
