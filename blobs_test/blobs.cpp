...
int Blobs::runlengthAnalysis()
{
    ...
    while(1)
    {
        while (m_qq->dequeue(&qval)==0)//把在图像中识别到的位置一个取出来
		{
			//printf("qval.m_col=%x\r\n",qval.m_col);//jingwenyi add 2016.3.23
		}
		//printf("222 qval.m_col=%x\r\n",qval.m_col);//jingwenyi add 2016.3.23
        if (qval.m_col>=0xfffe)
        {
        	printf("qval.m_col>=0xfffe 111\r\n");
			 break;
		}
           
		if (res<0)
			continue;
        if (qval.m_col==0)//表示新的一行开始了
        {
            prevStartCol = 0xffff;
            prevSig = 0;
            if (segmentSig)
			...
			{
				//g_chirpUsb->service(); //jingwenyi delete 2016.3.23
				icount = 0;
			}
#endif
            continue;
        ...
        sig = qval.m_col&0x07;//取出识别到的那种颜色
        ...
        {
        	printf("jingwenyi runlengthAnalysis\r\n");
         	qval.m_col >>= 3;
        	startCol = qval.m_col;//取出x的位置
...
//jwy add 2016.3.24
//从代码来看，是每次比较了连续的两个像素，如果两个像素都是同一个识别的颜色
//就把第二个个x 值记录
void Blobs::handleLine(uint8_t *line, uint16_t width)
{
    ...
    Qval newline;
    // new line
    m_qq->enqueue(&newline);//提示新的一行开始了
    x = 1;
