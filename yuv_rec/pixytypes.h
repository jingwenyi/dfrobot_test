...
struct RectA
{
    ...
    uint16_t m_xOffset;//这个是用户选择的位置相对于宽度的偏移地址
    uint16_t m_yOffset; //这个是用户选择的位置相对于高度的偏移地址
    uint16_t m_width;//这个宽度是用户选择的方框的宽度
    uint16_t m_height; //这个高度是用户选择的方框的高度
};
...
struct YUVPixel //jwy add 2016.5.3 for yuv
{
    YUVPixel()
    {
        m_y = m_u = m_v = 0;
