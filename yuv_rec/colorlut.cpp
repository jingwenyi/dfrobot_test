...
#include <stdio.h>  //jwy add 2016.4.5 for printf
#include <stdlib.h>
#include <math.h>
#include <string.h>
...
void print_colorSignature(ColorSignature& sig); //jwy add 2016.4.5

void printf_RuntimeSignature(RuntimeSignature& sig)//jwy add 2016.4.5
{
	printf("RuntimeSignature\r\n");
	printf("m_uMin:%x\r\n",sig.m_uMin);
	printf("m_uMax:%x\r\n",sig.m_uMax);
	printf("m_vMin:%x\r\n",sig.m_vMin);
	printf("m_vMax:%x\r\n",sig.m_vMax);
...
#ifdef USE_RBBG_RECOGNITION  //jwy add  2016.4.1  rggb 求发
    m_pixels = m_frame.m_pixels + (m_region.m_yOffset | 1)*m_frame.m_width + (m_region.m_xOffset | 1);
#elif defined USE_RGB_RECOGNITION  //求rgb 的像素点的位置
	m_pixels = m_frame.m_pixels + m_region.m_yOffset*m_frame.m_width*3 + ((m_region.m_xOffset)/3+1)*3;
#elif defined USE_YUV_RECOGNITION  //jwy add 2016.5.3 yuv 像素点的位置
    //yuv 的数据存放时分开的是，这里传入一帧的数据比较好
    m_pixels = m_frame.m_pixels;
	//记录y、u、v数据的起始地址(yuv的格式是420p) 
#if 0
    yuv_yidx = m_frame.m_width*m_region.m_yOffset + m_region.m_xOffset;
	yuv_uidx = m_frame.m_width*m_frame.m_height + (yuv_yidx/2);
	yuv_vidx = m_frame.m_width*m_frame.m_height + (m_frame.m_width*m_frame.m_height >> 2) + (yuv_yidx/2);
#else
	yuv_yidx = m_frame.m_width*m_region.m_yOffset + m_region.m_xOffset;
	yuv_uidx = m_frame.m_width*m_frame.m_height + (m_region.m_yOffset/2)*(m_frame.m_width/2) + (m_region.m_xOffset/2);
	yuv_vidx = m_frame.m_width*m_frame.m_height + (m_frame.m_width*m_frame.m_height >> 2) + 
										(m_region.m_yOffset/2)*(m_frame.m_width/2) + (m_region.m_xOffset/2);
...
bool IterPixel::next_yuv(UVPixel *uv, YUVPixel *yuv)
{
    if (m_points)
    {
        if (nextHelper_yuv(uv, yuv))
            return true; // working on the current block
        else // get new block
        {
            if (reset(false)) // reset indexes, increment m_i, get new block
                return nextHelper_yuv(uv, yuv);  // we have another block!
            else
                return false; // blocks are empty
        }
    }
    else
        return nextHelper_yuv(uv, yuv);
}
...
bool IterPixel::nextHelper_yuv(UVPixel *uv, YUVPixel *yuv) //jwy add 2016.5.3 for yuv
{
	//qDebug()<<"this is yuv nexthelper";
	int32_t y,u,v,c,uu,vv,miny=30;
	while(1)
	{
		if(m_x >= m_region.m_width)
		{
			m_x = 0;
			m_y++;
			m_yy += m_frame.m_width;
			if((m_y%2) == 0)
			{
				m_u += m_frame.m_width/2;
				m_v += m_frame.m_width/2;
			}
		}
		...
		if(m_y >= m_region.m_height)
			return false;
		y = m_pixels[m_yy + m_x];
		u = m_pixels[m_u + m_x/2];
		v = m_pixels[m_v + m_x/2];
		...
		if(uv)
		{
			...
			uu = ((y-u)<<CL_LUT_ENTRY_SCALE)/c;
        	
        	vv = ((v-u)<<CL_LUT_ENTRY_SCALE)/c;
		...
		//uu = (u<<CL_LUT_ENTRY_SCALE);
		//vv = (v<<CL_LUT_ENTRY_SCALE);
...
{
#ifdef USE_RBBG_RECOGNITION  //这是计算rggb 数据的u v 值 jwy add 2016.4.1
...
#elif defined  USE_RGB_RECOGNITION //for rgb

	int32_t r, g, b, u, v, c, miny=CL_MIN_Y;
	while(1)
	{
		if (m_x>=m_region.m_width)
        {
		...
		if (uv)
		{
        	...
        	u = ((r-g)<<CL_LUT_ENTRY_SCALE)/c;
        	
        	v = ((b-g)<<CL_LUT_ENTRY_SCALE)/c;
...
uint32_t IterPixel::averageYUV(uint32_t *pixels) //jwy add 2016.5.3 for yuv
{
	YUVPixel yuv;
	uint32_t y, u, v, n;
	reset();
	for (y=u=v=n=0; next_yuv(NULL, &yuv); n++)
	{
		y += yuv.m_y;
		u += yuv.m_u;
		v += yuv.m_v;		
...
void ColorLUT::calcRatios(IterPixel *ip, ColorSignature *sig, float ratios[])
{
    ...
    ip->reset();
#if  defined(USE_RBBG_RECOGNITION) || defined(USE_RGB_RECOGNITION)
    while(ip->next(&uv))//m_u=0x58e0,m_v=0x366
#elif defined(USE_YUV_RECOGNITION)
	while(ip->next_yuv(&uv))
#endif
    {
        if (uv.m_u>sig->m_uMin)
            counts[0]++;
    ...
    ratios[2] = (float)counts[2]/n;
    ratios[3] = (float)counts[3]/n;
	//printf("ratios[0]:%f,ratios[1]=%f,ratios[2]=%f,ratios[3]=%f\r\n",ratios[0],ratios[1],ratios[2],ratios[3]);
    // calc mean (because it's cheap to do it here)
...
void ColorLUT::iterate(IterPixel *ip, ColorSignature *sig)
{
    int32_t scale;
    float ratios[4];
	int32_t scale_sun1 = 0;
	int32_t scale_sun2 = 0;
    ...
    for (scale=1<<30, sig->m_uMin=sig->m_uMax=sig->m_vMin=sig->m_vMax=0; scale!=0; scale>>=1)
    {
    	scale_sun1 +=scale;
		scale_sun2 -=scale;
    	//printf("scale=%x,scale_sun1=%x,scale_sun2=%x\r\n",scale,scale_sun1,scale_sun2);
...
#if 0  //jwy add 2016.4.6
	if(sig->m_uMin == sig->m_uMax)
	{
		sig->m_uMin -= 1;
		sig->m_uMax += 1;
	}
	if(sig->m_vMin == sig->m_vMax)
	{
		sig->m_vMax += 1;
		sig->m_vMin -= 1;
	}
...
int ColorLUT::generateSignature(const Frame8 &frame, const RectA &region, uint8_t signum)
{
   ...
   // this is cool-- this routine doesn't allocate any extra memory other than some stack variables
	...
	//m_signatures[signum-1].m_rgb = ip.averageRgb();//jwy add 2016.04.26
...
void ColorLUT::updateSignature(uint8_t signum)
{
	...
	//qDebug()<<"m_rgbSat" << m_runtimeSigs[signum].m_rgbSat;

	//printf_RuntimeSignature(m_runtimeSigs[signum]);
...
void print_colorSignature(ColorSignature& sig)
{
	...
	printf("--m_type:%x\r\n",sig.m_type);
#else
	qDebug()<<"m_uMIn:" << sig.m_uMin << "m_uMax:" << sig.m_uMax << "m_uMean:" << sig.m_uMean 
			<< "m_vMin:" <<sig.m_vMin << "m_vMax:" << sig.m_vMax << "m_vMean:" << sig.m_vMean;
#endif
}
//add end
...
int ColorLUT::setSignature(uint8_t signum, const ColorSignature &sig)
{
	...
	print_colorSignature(m_signatures[signum-1]);
	updateSignature(signum);
	return 0;
}
...
#if defined(USE_RGB_RECOGNITION)|| defined(USE_RBBG_RECOGNITION)

#if 0  //jwy add 2016.4.5
...
#endif
            {
                ...
                for (sig=0; sig<CL_NUM_SIGNATURES; sig++)
                {
                    if (m_signatures[sig].m_uMin==0 && m_signatures[sig].m_uMax==0)
                        continue;
					//m_uMin=0x58dc,m_uMax=58e3,m_vMin=362,m_vMax=369 识别的红色
					//m_umin=0xffffc039,m_uMax=ffffc03b,m_vMin=fffffd2f5,m_vMax=fffffd2f5; 绿色
                            ...
                            (m_runtimeSigs[sig].m_vMin<v) && (v<m_runtimeSigs[sig].m_vMax))
                    {
						...
						//printf("-r=%x,g=%x,b=%x,u=%x,v=%x,bin=%x\r\n",r,g,b,u,v,bin);
		...
		for (u=0; u<1<<8; u++)
        {
            for (v=0; v<1<<8; v++)
            {
				...
				for (sig=0; sig<CL_NUM_SIGNATURES; sig++)
                {
					if (m_signatures[sig].m_uMin==0 && m_signatures[sig].m_uMax==0)
                        continue;
					if ((m_runtimeSigs[sig].m_uMin<uu) && (uu<m_runtimeSigs[sig].m_uMax) &&
                            (m_runtimeSigs[sig].m_vMin<vv) && (vv<m_runtimeSigs[sig].m_vMax))
					{
						uu = u;
						...
						bin = (uu<<CL_LUT_COMPONENT_SCALE)+ vv;
