...
#define USE_YUV_RECOGNITION   //定义了这个红，表示用yuv的数据进行识别
//add end
...
#define CL_LUT_COMPONENT_SCALE          8//6
...
#define CL_DEFAULT_SIG_RANGE			2.5f //2.5f  jwy modify 2016.4.5
...
public:
	...
	bool next_yuv(UVPixel *uv, YUVPixel *yuv=NULL);
    bool reset(bool cleari=true);
	uint32_t averageRgb(uint32_t *pixels=NULL);
	uint32_t averageYUV(uint32_t *pixels=NULL);//jwy add 2016.5.3 for yuv

private:
    bool nextHelper(UVPixel *uv, RGBPixel *rgb);
	bool nextHelper_yuv(UVPixel *uv, YUVPixel *yuv);

    Frame8 m_frame;
    RectA m_region;
    uint32_t m_x, m_y;
	//jwy add 2016.5.3 for yuv 
	uint32_t m_yy,m_u,m_v;
	uint32_t yuv_yidx, yuv_uidx, yuv_vidx;
	//add end
    uint8_t *m_pixels;
